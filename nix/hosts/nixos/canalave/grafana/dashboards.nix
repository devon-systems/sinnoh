  ...
  flake.nixosModules.canalave = {
      ...
      dashboard "sinnoh-kubernetes-workloads" "Kubernetes workloads" [
        ...
        (variable "pod" ''label_values(sinnoh_pod_workload_info{node=~"$node",namespace=~"$namespace",workload=~"$workload"}, pod)'')
      ] ([
          {
            ...
            type = "stat";
            targets = [(target ''up{job=~"cadvisor|kube-state-metrics"}'' "{{job}} {{instance}}") (target ''sum(up{job="cadvisor"}) == bool 2'' "Both kubelets reachable") (target ''kube_node_status_condition{job="kube-state-metrics",node=~"$node",condition="Ready",status="true"}'' "{{node}} Ready")];
            unit = "bool";
          }
          {
            title = "Pod placement";
    ...
    backups = dashboard "sinnoh-postgres-backups" "Postgres and backups" [nodeVariable] [
      ...
      }
      {
        ...
        type = "stat";
        targets = [(target ''up{${backup}}'' "{{instance}} exporter reachable") (target ''node_textfile_scrape_error{${backup}}'' "{{instance}} textfile error")];
        description = "Exporter reachable should be 1 and textfile error 0. The node selector applies only to host backup panels.";
      }
      {
